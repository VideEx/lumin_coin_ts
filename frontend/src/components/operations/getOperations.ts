...
export class GetOperations extends Operations {
    ...
    filters;
    currentIdCategory: null | Number | String;
    ...
    init() {
        ...
        this.getTable('today', null, null);
        // this.filters.setFiltersBtn('operations');
        this.filters.setFiltersBtn();

        let currentFilters: HTMLElement | null = document.getElementById('today');
        currentFilters?.classList.add('btn-secondary');
        currentFilters?.classList.remove('btn-outline-secondary');
    }

    getTable(period: string | null, dateFrom: String | null, dateTo: String | null) {
        ...
        let data: Promise<OperationResponseType[]|void> = this.getOperations(period, dateFrom, dateTo);


        let currentNum = 0;
        (data as Promise<OperationResponseType[]>).then(op => {
            op.forEach(item => {
                console.log(item)
                let tr: HTMLElement = document.createElement('tr');
                let opNum: HTMLElement = document.createElement('td');
    ...
    async deleteOperations(id: Number|null) {
        // @ts-ignore
        ...
        if (btn) {
            ...
            this.getTable(operationId, null, null);
        }
    ...
    showDeleteModal(id: Number|null) {
